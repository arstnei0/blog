...
const toggle = () => {
    setMode(mode => mode === 'light' ? 'dark' : (mode === 'dark' ? 'system' : 'light'))
    const m = mode()
    if (m === 'system') {
        const modeName = (globalThis.matchMedia && globalThis.matchMedia("(prefers-color-scheme: dark)").matches) ? 'dark' : 'light'
